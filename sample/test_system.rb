...
test_ok(`java -jar xruby-0.3.1.jar -s script_tmp -zzz` == 'true')
#java will remove '=' automatically from command line, so we have to change the test
#test_ok(`java -jar xruby-0.3.1.jar -s script_tmp -zzz=555` == '555')
test_ok(`java -jar xruby-0.3.1.jar -s script_tmp "-zzz=555"` == '555')
...
#java will remove '=' automatically from command line, so we have to change the test
#test_ok(`java -jar xruby-0.3.1.jar script_tmp -zzz=678` == '678')
test_ok(`java -jar xruby-0.3.1.jar script_tmp "-zzz=678"` == '678')
...
test_ok(`java -jar xruby-0.3.1.jar -x script_tmp` == 'nil')
#java will remove '=' automatically from command line, so we have to change the test
#test_ok(`java -jar xruby-0.3.1.jar -x script_tmp -zzz=555` == '555')
test_ok(`java -jar xruby-0.3.1.jar -x script_tmp "-zzz=555"` == '555')
...
#`java -jar xruby-0.3.1.jar -i.bak -pe 'sub(/^[0-9]+$/){$&.to_i * 5}' script_tmp`
`java -jar xruby-0.3.1.jar -i.bak -pe "sub(/^[0-9]+$/){$&.to_i * 5}" script_tmp`
done = true
